...
uint64_t pagecount(uint64_t memory_size, uint64_t page_size) {
  // ~50ns per test, not optimized
  // ~3.5ns per test, with -O1 flag
  /* int expo = get_expo(page_size); */
  /* return memory_size >> expo; */

  // ~9ns per test, both with and without optimization flag
  /* return memory_size / page_size; */

  // ~2.25ns per test with -O1 flag
  // ~46ns without the flag!
  int n = 0;
  while (page_size > 0) {
    n++;
    page_size  >>= 1;
  }
  return memory_size >> n;
}
